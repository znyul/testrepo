...
# is the name that gets added to the classname parameter.
    def  testSuiteVerify(testSuiteName)
            @testFlag = true;
            # Split the path name 
            testSuiteName = testSuiteName.split(",")[0]
...
# Test was flagged as having passed so format the output
    def testPassed(array)
        ...
        testName = array[lastItem - 1].split(")")[0]
        testSuiteVerify(array[@className].split("(")[1])
        #printf "%-40s PASS\n", testName
        if @xmlOut == true
...
# Test was flagged as being ingored so format the output
    def testIgnored(array)
        lastItem = array.length - 1
        testName = array[1].split(")")[0]
        reason = ""
        if lastItem >= 3
            reason = array.slice(3,lastItem).join(" ") + " at line: " + array[1].split(")")[1].split("TEST")[0]
        end
        testSuiteVerify(array[@className].split("(")[1])
        #printf "%-40s IGNORED\n", testName
        if @xmlOut == true
